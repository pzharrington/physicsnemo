...
class Trainer:
    ...
    def _setup_seeds(self, step: int = 0):
        ...
        * Within a model-parallel (domain) group, diffusion sigma values
          are kept consistent via broadcast (handled automatically by
          :class:`~physicsnemo.diffusion.DomainParallelSchedulerWrapper`
          when ``device_mesh`` is passed to the loss), not by sharing a
          seed, so that spatial noise generated by ``torch.randn_like``
          remains independent per shard.
    ...
    def _setup_loss(self) -> MSEDSMLoss | Callable[..., torch.Tensor]:
        r"""
        Create the loss function.

        For regression models, uses MSE loss. For diffusion models, creates
        an :class:`~physicsnemo.diffusion.metrics.losses.MSEDSMLoss` with an
        :class:`~physicsnemo.diffusion.noise_schedulers.EDMNoiseScheduler`.
        When domain parallelism is active, the domain mesh is passed so that
        sampled sigmas are broadcast across spatial shards automatically.

        Returns
        -------
        MSEDSMLoss | Callable[..., torch.Tensor]
            The loss function.
        """
        ...
        domain_mesh = (
            self.parallel_helper.mesh["domain"]
            if self.use_shard_tensor
            else None
        )
        loss_fn = build_diffusion_loss(
            self.net,
            loss_params,
            self.device,
            logger=self.logger,
    ...
    def train_step(self) -> torch.Tensor:
        ...
        for _ in range(self.num_accumulation_rounds):
            ...
            with torch.autocast("cuda", dtype=self.amp_dtype, enabled=self.enable_amp):
                ...
                # SigmaBinTracker is not supported with the modern
                # MSEDSMLoss (the sampled sigma is not exposed). It
                # remains functional for any future loss that returns
                # sigma, but is effectively disabled here.
                self.sigma_bin_tracker.update(loss, None)
        ...
        with torch.no_grad():
            for v_i, batch in enumerate(valid_iter):
                ...
                with torch.autocast(
                    ...
                    if self.net_name == "regression":
                        loss_kwargs["return_model_outputs"] = True
